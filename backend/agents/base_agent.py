...
============================================================================
All 6 browser-use agents (bdjobs, skilljobs, jobmedia, bdtechjobs, niyog, atb)
...
SITE_CONFIG: dict[str, dict] = {
    ...
    "bdtechjobs": {
        "name": "bdtechjobs",
        "label": "BD Tech Jobs",
        "domains": ["*.bdtechjobs.com", "bdtechjobs.com"],
        "url_template": "https://www.bdtechjobs.com",
        "url_with_location": "https://www.bdtechjobs.com?search={query}&location={location}",
    },
    "niyog": {
        "name": "niyog",
        "label": "Niyog",
        "domains": ["*.niyog.co", "niyog.co"],
        "url_template": "http://www.niyog.co",
        "url_with_location": "http://www.niyog.co",
    },
    "atb": {
        "name": "atb",
        "label": "ATB Jobs",
...
class BaseJobAgent:
    ...
    def __init__(
        ...
        self._setup_detail_extraction_tool()
    ...
    def _setup_extraction_tool(self) -> None:
        @self.tools.action(
            description=(
                ...
                "Extract ALL available fields including: title, company, location, job_type, deadline, "
                "salary, experience, requirements, responsibilities, benefits, skills, education, company_info."
            )
        )
        async def extract_jobs_via_js(browser_session) -> ActionResult:
            # Enhanced JS extraction - tries to get more fields from job cards
            ...
            js_code = """() => {
                try {
                    ...
                    // Helper to extract text content cleanly
                    const cleanText = (text) => {
                        if (!text) return '';
                        return text.trim().split(/\\n/)[0].trim();
                    };
                    
                    // Helper to find field by label pattern in card
                    const findField = (card, labels) => {
                        if (!card) return '';
                        const cardText = card.innerText || '';
                        for (const label of labels) {
                            const idx = cardText.toLowerCase().indexOf(label.toLowerCase());
                            if (idx !== -1) {
                                const after = cardText.substring(idx + label.length).trim();
                    ...
                    for (const link of links) {
                        ...
                        const card = link.closest('div, li, article, section, tr, td, a');
                        const cardText = card ? card.innerText : '';
                        const lines = cardText.split(/\\n/).map(l => l.trim()).filter(l => l.length > 0);
                        
                        // Try to find company
                        let company = findField(card, ['Company:', 'Company Name:', 'Employer:', 'Organization:']);
                        if (!company && lines.length > 1) {
                            const secondLine = lines[1];
                            if (secondLine && !secondLine.match(/\\d{4}|Deadline|Posted|Job Type|Type:/) && secondLine.length > 2) {
                                company = secondLine.substring(0, 80);
                            }
                        }
                        
                        // Try to find location
                        let location = findField(card, ['Location:', 'Address:', 'Area:', 'City:', 'Work Place:']);
                        if (!location && lines.length > 2) {
                            const thirdLine = lines[2];
                            if (thirdLine && !thirdLine.match(/\\d{4}|Posted|Deadline/)) {
                                location = thirdLine.substring(0, 80);
                            }
                        }
                        
                        const jobType = findField(card, ['Job Type:', 'Type:', 'Employment Type:', 'Contract', 'Full Time', 'Part Time']);
                        const deadline = findField(card, ['Deadline:', 'Last Date:', 'Closing Date:', 'Application Deadline:']);
                        const salary = findField(card, ['Salary:', 'Pay:', 'Remuneration:', 'TK', 'BDT', 'Negotiable']);
                        const experience = findField(card, ['Experience:', 'Exp:', 'Years of Experience:', 'Work Experience:']);
                        const requirements = findField(card, ['Requirements:', 'Qualifications:', 'Skill:', 'Skills Required:', 'Who Can Apply:']);
                        const responsibilities = findField(card, ['Responsibilities:', 'Duties:', 'Job Description:']);
                        const benefits = findField(card, ['Benefits:', 'Perks:', 'What We Offer:', 'Compensation:']);
                        const education = findField(card, ['Education:', 'Academic:', 'Degree:', 'Educational Requirement:']);
                        
                        // Extract skill tags - simplified selector
                        let skills = '';
                        try {
                            if (card) {
                                const skillTags = card.querySelectorAll('.skill, .tag, [class*=\"skill\"], [class*=\"tag\"], [class*=\"tech\"], [class*=\"keyword\"]');
                                if (skillTags && skillTags.length > 0) {
                                    skills = Array.from(skillTags).map(s => s.innerText).filter(t => t && t.length > 0 && t.length < 30 && !/^[0-9]+$/.test(t)).join(', ');
                                }
                            }
                        } catch(e) {
                            // Skip skill extraction on error
                        }
                    ...
                    return JSON.stringify({error: e.message, jobs: []});
                }
            }"""
            try:
                ...
                # Handle error object from wrapped JS
                if isinstance(raw, str) and raw.startswith("{"):
                    try:
                        import json
                        ...
                        if "error" in parsed:
                            print(f"[extract_jobs_via_js] JS Error: {parsed['error']}")
                            return ActionResult(extracted_content="[]")
                        if "jobs" in parsed:
                            return ActionResult(
                                extracted_content=json.dumps(parsed.get("jobs", []))
                            )
                    except:
                        pass
                return ActionResult(extracted_content="[]")
    ...
    def _setup_detail_extraction_tool(self) -> None:
        """Extract detailed info from job detail pages - for better matching accuracy."""

        @self.tools.action(
            description=(
                "Extract COMPLETE job details from a job detail page. "
                "Use this AFTER clicking on a job to get full information including: "
                "requirements, responsibilities, benefits, skills, education, company info. "
                "Returns a JSON object with ALL available job fields."
            )
        )
        async def extract_job_details(browser_session) -> ActionResult:
            js_code = """() => {
                // Helper to extract text after a label
                const extractLabel = (label) => {
                    const allText = document.body.innerText;
                    const idx = allText.toLowerCase().indexOf(label.toLowerCase());
                    if (idx === -1) return '';
                    
                    // Get text after label, up to next section or 500 chars
                    let after = allText.substring(idx + label.length).trim();
                    // Find common section breaks
                    const breakIdx = after.search(/\\n\\n|Requirements|Responsibilities|Benefits|Education|Job Description|---/i);
                    if (breakIdx > 0 && breakIdx < 500) {
                        after = after.substring(0, breakIdx).trim();
                    }
                    return after.substring(0, 500);
                };
                
                // Get page title
                const title = document.querySelector('h1, h2, [class*="title"], [class*="job-title"]')?.innerText || '';
                
                // Get company name
                const company = extractLabel('Company') || extractLabel('Employer') || document.querySelector('[class*="company"]')?.innerText || '';
                
                // Get location
                const location = extractLabel('Location') || extractLabel('Address') || document.querySelector('[class*="location"]')?.innerText || '';
                
                // Get job type
                const jobType = extractLabel('Job Type') || extractLabel('Employment Type') || extractLabel('Type') || '';
                
                // Get deadline
                const deadline = extractLabel('Deadline') || extractLabel('Last Date') || extractLabel('Closing Date') || '';
                
                // Get salary
                const salary = extractLabel('Salary') || extractLabel('Pay') || extractLabel('Remuneration') || '';
                
                // Get experience
                const experience = extractLabel('Experience') || extractLabel('Exp') || '';
                
                // Get requirements
                const requirements = extractLabel('Requirements') || extractLabel('Qualifications') || extractLabel('Skills Required') || '';
                
                // Get responsibilities
                const responsibilities = extractLabel('Responsibilities') || extractLabel('Duties') || extractLabel('Job Description') || '';
                
                // Get benefits
                const benefits = extractLabel('Benefits') || extractLabel('Perks') || extractLabel('What We Offer') || '';
                
                // Get education
                const education = extractLabel('Education') || extractLabel('Academic') || extractLabel('Degree') || '';
                
                // Get skills from tags
                const skillElements = document.querySelectorAll('[class*="skill"], [class*="tag"], .chip, .badge, [class*="tech"]');
                const skills = Array.from(skillElements).map(el => el.innerText.trim()).filter(t => t.length > 0 && t.length < 30).join(', ');
                
                // Get company info
                const companyInfo = extractLabel('About') || extractLabel('Company') || extractLabel('About Company') || '';
                
                // Get URL
                const url = window.location.href;
                
                return JSON.stringify({
                    title, url, company, location, job_type: jobType, 
                    deadline, salary, experience, requirements, responsibilities,
                    benefits, skills, education, company_info: companyInfo
                });
            }"""
            try:
                page = await browser_session.get_current_page()
                ...
                print(f"[extract_job_details] Error: {e}")
                return ActionResult(error=f"Detail extraction failed: {e}")
    ...
    def _normalise(self, j: Any) -> dict | None:
        ...
        if not cleaned.get("skills"):
            cleaned["skills"] = (
                cleaned.get("skills")
                or cleaned.get("Skills")
                or cleaned.get("required_skills")
                or cleaned.get("key_skills")
                or cleaned.get("tech_stack")
                or ""
            )

        # Education variations
        if not cleaned.get("education"):
            cleaned["education"] = (
                cleaned.get("education")
                or cleaned.get("Education")
                or cleaned.get("educational_requirements")
                or cleaned.get("qualification")
                or ""
            )
        ...
        if not cleaned.get("industry"):
            cleaned["industry"] = (
                ...
                or cleaned.get("Industry")
                or cleaned.get("category")
                or cleaned.get("job_category")
                or ""
            )

        # Company info variations
        if not cleaned.get("company_info"):
            cleaned["company_info"] = (
                cleaned.get("company_info")
                or cleaned.get("about_company")
                or cleaned.get("company_description")
                or ""
            )
    ...
    def _resolve_relative_url(self, url: str) -> str:
        ...
        domain_map = {
            ...
            "bdtechjobs": "https://www.bdtechjobs.com",
            "niyog": "http://www.niyog.co",
    ...
    def _is_relevant(self, job: dict, query: str) -> bool:
        """Check if job title is relevant to the search query."""
        if not query:
            return True
        ...
        query_words = {w.strip() for w in query_words if w.strip() and len(w) > 1}

        title_words = set(
            title.replace("/", " ").replace("-", " ").replace("_", " ").split()
        )
        title_words = {w.strip() for w in title_words if w.strip() and len(w) > 1}

        matched = query_words & title_words
        min_matches = 1 if len(query_words) <= 2 else min(2, len(query_words) // 2)
    ...
    def _merge_jobs(self, new_jobs: list[Any]) -> int:
        ...
        for raw in new_jobs:
            ...
            if not self._is_relevant(j, self.query):
                print(
                    f"[_merge_jobs] Skipped irrelevant job: {j.get('title')} (query: {self.query})"
                )
                continue
    ...
    def _create_browser(self) -> Browser:
        ...
        return Browser(
            ...
            disable_security=True,  # Disable security to ignore SSL errors
            args=[
    ...
    def _get_system_message(self, site_name: str) -> str:
        ...
        return (
            ...
            "7. EXTRACT ALL AVAILABLE FIELDS: title, company, location, job_type, deadline, salary, experience, requirements, responsibilities, benefits, skills, education.\n"
            "8. When you click on a job detail page, ALWAYS call 'extract_job_details' to get complete job information including requirements, responsibilities, benefits, skills, education.\n"
            "9. Accuracy is critical - extract as much detail as possible from each job to improve matching quality.\n"
        )
