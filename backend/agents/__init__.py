...
AGENTS: dict[str, dict] = {
    "bdjobs": {
        "label": "BDJobs",
        "script": None,       # set to Path if subprocess; None = in-process module
        "module": "agents.bdjobs_agent",
    },
    # "linkedin": { "label": "LinkedIn", "module": "agents.linkedin_agent" },
}

def available_agents() -> list[dict]:
    return [{"key": k, "label": v["label"]} for k, v in AGENTS.items()]

async def run_agent(agent_key: str, query: str, location: str,
                    max_jobs: int, search_id: int, user_id: int) -> dict:
    ...
    if cfg.get("script"):
        return await _run_subprocess(cfg["script"], query, location, max_jobs,
                                     search_id, user_id)

    # ── in-process module path ────────────────────────────────────────────────
    mod = importlib.import_module(cfg["module"])
    return await mod.run(query=query, location=location, max_jobs=max_jobs,
                         search_id=search_id, user_id=user_id)

async def _run_subprocess(script: Path, query, location, max_jobs,
                           search_id, user_id) -> dict:
    args = [
        sys.executable, str(script),
        "--query", query,
        "--location", location or "",
        "--max-jobs", str(max_jobs),
        "--search-id", str(search_id),
        "--user-id", str(user_id),
    ]
    ...
    try:
        ...
        return {"status": "failed", "total": 0, "jobs": [], "error": stderr.decode()[:500]}
