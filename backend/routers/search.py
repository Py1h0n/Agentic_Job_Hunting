...
STANDARD_AGENTS = [
    ...
    "skilljobs",
    "bdjobs",
]  # Standard: 3 parallel sources with live streaming
DEEP_AGENTS = [  # Deep: 4 sources, semaphore of 2
    "bdjobs",
    "skilljobs",
    "jobmedia",
    "bdtechjobs",
...
def _get_standard_agents() -> list[str]:
    """Standard mode: 3 parallel sources with live streaming"""
    return STANDARD_AGENTS.copy()


def _get_deep_agents() -> list[str]:
    """Deep mode: 4 sources, semaphore of 2"""
...
class SearchReq(BaseModel):
    ...
    location: str = ""
    max_jobs: int = 30
    agent: str | None = None  # legacy single-agent override
    mode: str = "turbo"  # keep for backward compatibility
    agents: list[str] | None = None  # NEW: explicit agent list (overrides mode)
...
@router.get("/agents")
def list_agents():
    """List all available agents for search."""
...
async def create_search(body: SearchReq, user=Depends(current_user)):
    ...
    # Determine which agents to run - explicit agents take precedence
    # Check for non-empty list explicitly since empty list [] is falsy in Python
    if body.agents and len(body.agents) > 0:
        # Validate explicit agent list
        ...
        agents_to_run = body.agents
    else:
        # Fall back to mode-based preset
        if body.mode == "deep":
    ...
    # For deep mode, use semaphore of 2 (session-based concurrency)
    session_semaphore = 3  # default
    if body.mode == "deep":
        session_semaphore = 2
...
def list_modes():
    return {
        "modes": [
            ...
            {
                "key": "standard",
                "label": "Standard (3 parallel with live streaming)",
                "agents": _get_standard_agents(),
            },
            {
                "key": "deep",
                "label": "Deep (4 sources, 2 at a time)",
