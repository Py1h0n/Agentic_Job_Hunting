...
# Session-based semaphores for controlled concurrency
...
def get_session_semaphore(session_id: str, limit: int = 3) -> asyncio.Semaphore:
    """Get or create a session-specific semaphore for controlled concurrency."""
    global _session_semaphores
    if session_id not in _session_semaphores:
        _session_semaphores[session_id] = asyncio.Semaphore(limit)
...
async def _worker():
    ...
    while True:
        item = await q.get()
        # Handle both old format (6 items) and new format (8 items)
        if len(item) == 6:
...
async def _run(
    ...
    # Use session semaphore if provided for controlled concurrency (deep mode)
    if session_id:
        ses_sem = get_session_semaphore(session_id, session_semaphore)
        async with global_sem:
            async with ses_sem:
                await _execute_agent(
                    search_id, user_id, agent, query, location, max_jobs
                )
    else:
        async with global_sem:
            await _execute_agent(search_id, user_id, agent, query, location, max_jobs)


async def _execute_agent(search_id, user_id, agent, query, location, max_jobs):
